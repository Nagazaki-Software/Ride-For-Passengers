...
class MapViewController: NSObject, FlutterPlatformView, GMSMapViewDelegate, GMUClusterRendererDelegate {
  ...
  private func applyInitial(params: [String: Any]) {
    ...
    if let mapId = params["mapId"] as? String, !mapId.isEmpty {
      if mapView.responds(to: NSSelectorFromString("setMapID:")) {
        (mapView as NSObject).setValue(GMSMapID(identifier: mapId), forKey: "mapID")
      }
    }
  ...
  private func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "map#setMapId":
      if let mapId = call.arguments as? String {
        if mapView.responds(to: NSSelectorFromString("setMapID:")) {
          (mapView as NSObject).setValue(GMSMapID(identifier: mapId), forKey: "mapID")
        }
      }
