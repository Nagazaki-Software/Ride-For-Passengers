  ...
  try {
    ...
    if (user == null) {
      ...
      if (doCharge) {
        ...
        // Suporte a token de método de pagamento (vaulted)
        String? paymentMethodToken = (payload['token'] is String)
            ? (payload['token'] as String)
            : (payload['paymentMethodToken'] is String)
                ? (payload['paymentMethodToken'] as String)
                : null;

        if (paymentMethodToken != null && paymentMethodToken.isNotEmpty) {
          final isSandbox = tk.startsWith('sandbox_');
          final functions = FirebaseFunctions.instanceFor(region: 'us-central1');
          Future<Map<String, dynamic>> _callByName(String name) async {
          ...
          try {
            final fn = isSandbox
                ? 'processBraintreeTestPaymentWithToken'
                : 'processBraintreePaymentWithToken';
            final data = await _callByName(fn);
            ...
            // Se a function específica não existir, tenta a genérica
            if ((e.code == 'not-found') ||
