...
class CardData {
  ...
  String get month => (expiry.split('/').firstOrNull ?? '').trim().padLeft(2, '0');
  String get year {
...
class _CardFormFFState extends State<CardFormFF> {
  ...
  Widget build(BuildContext context) {
    final ctaText = widget.chargeOnConfirm
  ...
  Future<void> _onConfirmPressed() async {
    ...
    if (!widget.chargeOnConfirm) {
      ...
      try {
        ...
        final jsonReturn = <String, dynamic>{
          ...
          'last4': saved.last4 ?? (data.number.length >= 4
        ...
        try {
          if (widget.onSave != null) {
            ...
            await widget.onTextField!({'event': 'saved', 'json': jsonReturn});
          }
        } catch (_) {}

        showSnackbar(context, 'Saved.');
        // Do not auto-close on save to respect parent flow
      } catch (e) {
    ...
    try {
      ...
      try {
        ...
        if (kDebugPayments) debugPrint('Braintree PlatformException');
        showSnackbar(context, _friendlyBraintreeError(pe));
        return;
      } catch (e) {
        if (kDebugPayments) debugPrint('Braintree unexpected error: ' + e.toString());
        showSnackbar(
          context,
          e is MissingPluginException
              ? 'Payments not available on this build. Please reinstall or check plugins.'
  ...
  Future<void> _emitState({String event = 'change', String? mode}) async {
    ...
    try { await widget.onTextField!(payload); } catch (_) {}
  }
  ...
  String _detectBrand(String digits) {
    ...
    if (RegExp(r'^(5[1-5])').hasMatch(digits) || RegExp(r'^(2(2[2-9]|[3-6]\d|7[01]|720))').hasMatch(digits)) return 'mastercard';
  ...
  Widget _cardNumberField() {
    return TextFormField(
      ...
      onChanged: (_) { setState(() {}); _emitState(event: 'change'); },
      validator: (v) => _validateNumber(_digitsOnly(v ?? '')),
      style: const TextStyle(color: Colors.white),
      decoration: _inputDecoration(hint: 'Card number', icon: Icons.credit_card),
    );
  }
  Widget _expiryField() {
    return TextFormField(
      ...
      style: const TextStyle(color: Colors.white),
      decoration: _inputDecoration(hint: 'Expiry (MM/YY)', icon: Icons.date_range),
    );
  }
  Widget _cvvField() {
    return TextFormField(
      ...
      style: const TextStyle(color: Colors.white),
      decoration: _inputDecoration(hint: 'CVV', icon: Icons.lock_outline_rounded),
    );
  }
  Widget _holderField() {
    return TextFormField(
      ...
      style: const TextStyle(color: Colors.white),
      decoration: _inputDecoration(hint: 'Name on card', icon: Icons.person_outline),
    );
  }
  ...
  Widget _pillButton({required String label, required Future<void> Function()? onTap, bool loading = false}) {
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          child: loading
              ...
              : Text(label, style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w700, color: const Color(0xFF1E1E1E), letterSpacing: 0.2)),
        ),
      ),
    );
  }
  Widget _secureBadge() {
    return Semantics(
      ...
      child: Row(children: const [
        ...
        SizedBox(width: 8),
        Expanded(child: Text('Secure connection. Your data is encrypted.', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontFamily: 'Poppins', fontSize: 13, color: Colors.white70, height: 1.2))),
      ]),
    );
  }
        ...
        return Padding(
          ...
          child: Container(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const SizedBox(height: 12),
              Text('Payment approved!', style: GoogleFonts.poppins(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w600)),
              const SizedBox(height: 8),
              Text('Pass: $passe\nTransaction: ${_transactionId ?? '-'}', textAlign: TextAlign.center, style: GoogleFonts.poppins(color: Colors.white70)),
              const SizedBox(height: 16),
              _pillButton(label: 'Close', onTap: () async { Navigator.pop(context); }),
            ]),
          ),
        );
      },
        ...
        return Padding(
          ...
          child: Container(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const SizedBox(height: 12),
              Text("We couldn't process your payment.", textAlign: TextAlign.center, style: GoogleFonts.poppins(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600)),
              if (message != null && message.trim().isNotEmpty) ...[
                const SizedBox(height: 8),
                Text(message, textAlign: TextAlign.center, style: GoogleFonts.poppins(color: Colors.white70, fontSize: 13)),
              ],
              const SizedBox(height: 16),
              _pillButton(label: 'Try again', onTap: () async { Navigator.pop(context); }),
            ]),
          ),
        );
      },
  ...
  Future<void> _dismiss() async { if (Navigator.canPop(context)) { Navigator.of(context).pop(); } }

  Future<PaymentResponse> _callProcessPaymentCallable({required double amount, required String paymentNonce, bool isProd = false}) async {
    try {
      ...
      final resp = await callable.call(<String, dynamic>{'amount': amount.toStringAsFixed(2), 'paymentNonce': paymentNonce});
      ...
      if (kDebugPayments) debugPrint('[PAY][FunctionsException] $msg');
  ...
  Future<SaveMethodResponse> _callSavePaymentMethodCallable({required String nonce, bool isProd = false}) async {
    try {
      final functions = FirebaseFunctions.instanceFor(region: 'us-central1');
      final name = isProd ? 'savePaymentMethod' : 'savePaymentMethodTest';
      final callable = functions.httpsCallable(name);
      final resp = await callable.call(<String, dynamic>{'nonce': nonce});
      final data = Map<String, dynamic>.from(resp.data as Map);
      return SaveMethodResponse(token: data['token'] as String?, last4: data['last4'] as String?, name: data['name'] as String?);
    } on FirebaseFunctionsException catch (e) {
      final parts = <String>[];
      if (e.message != null && e.message!.trim().isNotEmpty) parts.add(e.message!.trim());
      if (e.details != null && e.details.toString().trim().isNotEmpty) parts.add(e.details.toString().trim());
      parts.add('CODE: ${e.code.toUpperCase()}');
      final msg = parts.join(' · ');
      if (kDebugPayments) debugPrint('[SAVE][FunctionsException] $msg');
...
class _ExpiryDateInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    if (digits.length >= 2) {
      ...
      if (m == 0) mm = '01'; else if (m > 12) mm = '12';
